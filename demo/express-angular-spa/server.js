/**
 * Express API server — awesome-node-auth backend for the Express + Angular SPA demo.
 *
 * Development (npm start):
 *   Runs on port 3000. Angular dev server (port 4200) proxies /auth/* and
 *   /admin/* to this server via proxy.conf.json.
 *
 * Production (npm run serve:prod after npm run build):
 *   Also serves the compiled Angular SPA from dist/ so a single process
 *   handles everything.
 */
...
const distPath = path.join(__dirname, 'dist', 'awesome-node-auth-express-angular-spa', 'browser');
const fs = require('fs');
